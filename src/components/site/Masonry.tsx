...
const useMedia = (queries: string[], values: number[], defaultValue: number): number => {
  ...
  useEffect(() => {
    ...
    return () => mqls.forEach((mq) => mq.removeEventListener('change', handler));
  }, [queries]);
  ...
  return (
    <div ref={containerRef} className="relative w-full" style={{ height: `${containerHeight}px` }}>
      {grid.map((item) => (
        ...
        >
          <div
            className="relative w-full h-full bg-cover bg-center rounded-[16px] shadow-[0px_10px_30px_-20px_rgba(0,0,0,0.35)] overflow-hidden"
